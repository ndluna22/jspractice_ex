...
function createAccount(pin, amount) {
  return {
    checkBalance(newPin) {
      //if pin is invalid, then return "invalid pin"
      if (newPin !== pin) return "Invalid PIN.";
      return `$${amount}`; //return balance
    },
    deposit(newPin, newAmount) {
      //if pin is invalid, then return "invalid pin"
      if (newPin !== pin) return "Invalid PIN.";
      amount += newAmount;
      return `Succesfully deposited $${newAmount}. Current balance: $${amount}.`;
    },
    withdraw(newPin, withdrawalAmount) {
      //if pin is invalid, then return "invalid pin"
      if (newPin !== pin) return "Invalid PIN.";
